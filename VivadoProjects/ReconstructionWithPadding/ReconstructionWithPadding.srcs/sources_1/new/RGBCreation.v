...
always @(*)
    begin
         if ((i_rows[0] == 1) && (j_cols[0] == 0)) // Blue Color -- Odd i's && even j's
            begin 
                RoutFinal = RBout_M1;
                GoutFinal = {GoutInterPolated[10-1], GoutInterPolated};
                BoutFinal = R7;
            end
        else if ((i_rows[0] == 0) && (j_cols[0] == 1)) // Red Color -- Even i's && odd j's
            begin 
                RoutFinal = R7;
                GoutFinal = {GoutInterPolated[10-1], GoutInterPolated};
                BoutFinal = RBout_M1;
            end
        else if ((i_rows[0] == 1) && (j_cols[0] == 1)) // Green Color (BGBG...) -- Odd i's && odd j's
            begin 
                RoutFinal = {RBout_M2[10-1], RBout_M2};
                GoutFinal = R7;
                BoutFinal = {{2{RBout_M3[9-1]}}, RBout_M3};
            end
        else if ((i_rows[0] == 0) && (j_cols[0] == 0)) // Green Color (GRGR...) -- Even i's && even j's
            begin 
